...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        if (createLocale) {                 // this needs only once, to create
            ...
            } // if none don't worry, it defaults to English
            EnterFrame EF = new EnterFrame();    // create the form 
            ...
            if (fileToCheck.length() > 0) EF.initializeTournament(EF, fileToCheck);
                ...
                if (!EF.initializeTournament()) {
                    EF.dispose();
                    return;
                }         // show modal dialog for individual/team/open tournament
                ...
                if (w < w2) {
                    d.setSize(w2, d.height);
                    EF.setMinimumSize(d);
                    EF.setPreferredSize(d);
                    EF.invalidate();
                }
            }
        }
                            ...
                            if (englishText != null)
                                if (englishText.trim().compareTo("") != 0)
                                    if (createLocale) registerText(englishText);
                                        ...
                                        ((JTable) C).getColumnModel().getColumn(i).setHeaderValue(localizedText(englishText));
                        }
